...
expName = 'test_circle'  # from the Builder filename that created this script
...
while continueRoutine and routineTimer.getTime() > 0:
    ...
    if ring.status == STARTED:
        ...
        if tThisFlipGlobal > ring.tStartRefresh + 1.0-frameTolerance:
            # keep track of stop time/frame for later
            ring.tStop = t  # not accounting for scr refresh
            ring.frameNStop = frameN  # exact frame index
            win.timeOnFlip(ring, 'tStopRefresh')  # time at next scr refresh
...
thisExp.addData('ring.started', ring.tStartRefresh)
thisExp.addData('ring.stopped', ring.tStopRefresh)
