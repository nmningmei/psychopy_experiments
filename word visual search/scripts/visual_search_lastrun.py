...
expInfo = {'participant': 'test', 'session': '001'}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\ning\\Documents\\python works\\psychopy_experiments\\word visual search\\scripts\\visual_search_lastrun.py',
...
for thisTrial in trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if targets.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > targets.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                targets.tStop = t  # not accounting for scr refresh
                targets.frameNStop = frameN  # exact frame index
                win.timeOnFlip(targets, 'tStopRefresh')  # time at next scr refresh
