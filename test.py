...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        fixDuration = np.random.random()
        frameRemains = 0.0 + fixDuration +0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
        if fixation.status == STARTED and t >= frameRemains:
            fixation.setAutoDraw(False)
        
        # *text* updates
        if t >= 0.5+ fixDuration and text.status == NOT_STARTED:
            # keep track of start time/frame for later
            text.tStart = t
            text.frameNStart = frameN  # exact frame index
            text.setAutoDraw(True)
        frameRemains = 0.5+np.random.random() + 2- win.monitorFramePeriod * 0.75  # most of one frame period left
